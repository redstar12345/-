...
from stu_course import Ui_Form

class Stu_courseUIForm(QMainWindow, Ui_Form):
    def __init__(self, parent=None):
        super(Stu_courseUIForm, self).__init__(parent)
        self.setupUi(self)
        self.sqlset = ""
        self.pages = [0]
        self.pages[0] = 0
        ...
        self.course_Button.clicked.connect(lambda: self.display(self.pages, -2000000))
    ...
    def display(self, pages, sign):
        ...
        str_pages = str(num)
        if self.sqlset == "":
            sql = "select * from course limit 8 offset " + str_pages
        else:
    ...
    def change(self, local):
        ...
        course_ID = textBrowser_change_obj.toPlainText()
        self.close()
        os.system("python call_change_course.py " + course_ID)
    ...
    def delete(self, local):
        ...
        sql = "use student_sys_db"
        cursor.execute(sql)
        sql = "select * from achievement where csid = " + course_ID
        cursor.execute(sql)
        data = cursor.fetchall()
        row = len(data)
        if row != 0:
            print("外键约束！数据不可删除！")
            return
        sql = "delete from course where csid = " + course_ID
        cursor.execute(sql)
        conn.commit()
        self.close()
        os.system("python call_delete_hint.py call_stu_course")
    ...
    def checkButton(self):
        self.close()
        os.system("python call_select_course.py")

    def addButton(self):
        self.close()
        os.system("python call_add_course.py")

    def display_check(self):
        # 由于 call_select_course.py 的限制，这里至少有一个参数
        if_and = False
        sql = "select course.csid, course.csname, course.cstime, course.credit from course where "
        if sys.argv[1] != "course.csid=":
            if if_and == False:
                sql = sql + sys.argv[1]
                if_and = True
            else:
                sql = sql + " and " + sys.argv[1]
        if sys.argv[2] != "course.csname=''":
            if if_and == False:
                tempargv = "course.csname like '%" + sys.argv[2][15:len(sys.argv[2]) - 1] + "%'"
                sql = sql + tempargv
                if_and = True
            else:
                tempargv = "course.csname like '%" + sys.argv[2][15:len(sys.argv[2]) - 1] + "%'"
                sql = sql + " and " + tempargv
        if sys.argv[3] != "course.cstime=":
            if if_and == False:
                sql = sql + sys.argv[3]
                if_and = True
            else:
                sql = sql + " and " + sys.argv[3]
        if sys.argv[4] != "course.credit=":
            if if_and == False:
                sql = sql + sys.argv[4]
                if_and = True
            else:
