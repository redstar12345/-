...
from stu_class import Ui_Form

class Stu_classUIForm(QMainWindow, Ui_Form):
    def __init__(self, parent=None):
        super(Stu_classUIForm, self).__init__(parent)
        self.setupUi(self)
        self.sqlset = ""
        self.pages = [0]
        self.pages[0] = 0
        ...
        self.class_BUTTON.clicked.connect(lambda: self.display(self.pages, -2000000))
        ...
        self.promessage_BUTTON.clicked.connect(self.promessage)
        self.add_BUTTON.clicked.connect(self.addButton)

    def display(self, pages, sign):
        if sign == -2000000:
            self.sqlset = "" # class 信号
        conn = pymysql.connect(host = '127.0.0.1', port = 3306, user = 'root', passwd = 'Dth20030116', charset = 'utf8')
        cursor = conn.cursor()
        sql = "use student_sys_db"
        cursor.execute(sql)
        ...
        str_pages = str(num)
        if self.sqlset == "":
            sql = "select class.cid, class.cname, class.num, pro.pname from class left join pro on class.pid = pro.pid limit 8 offset " + str_pages
        else:
    ...
    def change(self, local):
        ...
        class_ID = textBrowser_change_obj.toPlainText()
        self.close()
        os.system("python call_change_class.py " + class_ID)
    ...
    def delete(self, local):
        ...
        if row != 0:
            print("外键约束！数据不可删除！")
            return
        sql = "delete from class where cid = " + class_ID
        cursor.execute(sql)
        conn.commit()
        self.close()
        os.system("python call_delete_hint.py call_stu_class")
    ...
    def checkButton(self):
        self.close()
        os.system("python call_select_class.py")

    def addButton(self):
        self.close()
        os.system("python call_add_class.py")

    def display_check(self):
        # 由于 call_select_class.py 的限制，这里至少有一个参数
        if_and = False
        sql = "select class.cid, class.cname, class.num, pro.pname from class left join pro on class.pid = pro.pid where "
        if sys.argv[1] != "class.cid=":
            if if_and == False:
                sql = sql + sys.argv[1]
                if_and = True
            else:
                sql = sql + " and " + sys.argv[1]
        if sys.argv[2] != "class.cname=''":
            if if_and == False:
                tempargv = "class.cname like '%" + sys.argv[2][13:len(sys.argv[2]) - 1] + "%'"
                sql = sql + tempargv
                if_and = True
            else:
                tempargv = "class.cname like'%" + sys.argv[2][13:len(sys.argv[2]) - 1] + "%'"
                sql = sql + " and " + tempargv
        if sys.argv[3] != "class.num=":
            if if_and == False:
                sql = sql + sys.argv[3]
                if_and = True
            else:
                sql = sql + " and " + sys.argv[3]
        if sys.argv[4] != "pro.pname=''":
            if if_and == False:
                tempargv = "pro.pname like '%" + sys.argv[4][11:len(sys.argv[4]) - 1] + "%'"
                sql = sql + tempargv
                if_and = True
            else:
                tempargv = "pro.pname like '%" + sys.argv[4][11:len(sys.argv[4]) - 1] + "%'"
